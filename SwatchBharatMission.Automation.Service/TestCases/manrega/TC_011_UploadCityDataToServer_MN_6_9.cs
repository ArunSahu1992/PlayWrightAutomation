...
namespace TestCases
{
    public class TC_011_UploadCityDataToServer_MN_6_9 : ITestCase
    {
        public string TestCaseId => "TC_011_UploadCityDataToServer_MN_6_9";
        ...
        public async Task ExecuteAsync(AutomationContext automationContext, ILogger logger)
        {
            ...
            try
            {
                ...
                {
                    HasText = "MGNREGS daily status as per e-muster issued"
                }).ClickAsync();
                ...
                var cityLinks = page.Locator("table tr td:nth-child(2) a:not(:has-text('Total'))");
                ...
                if (cityCount == 0)
                {
                    ...
                    cityLinks = page.Locator("table tr td:nth-child(2) a");
                ...
                try
                {
                    for (int i = 0; i < cityCount; i++)
                    {
                        ...
                        var cityLink = page.Locator("table tr td:nth-child(2) a").Nth(i);

                        string cityName = (await cityLink.InnerTextAsync()).Trim();
                        if (cityName == string.Empty) continue;
                        ...
                        // Go back
                        await page.ClickAsync("#ContentPlaceHolder1_back");
